...
internal_adhoc:
	...
	flutter build ipa --release --flavor internal --target lib/main_internal.dart --export-options-plist=ios/ExportOptions-internal-debug.plist
...
external_adhoc:
	...
	cd ios && pod install && cd ..
	echo "╠ Releasing to pgyer..."
	flutter build ipa --release --flavor external --target lib/main_external.dart --export-options-plist=ios/ExportOptions-external-debug.plist
